...
try:
    ...
    st.error("🔑 ERREUR DEPLOYEMENT: Secrets introuvables. Vérifiez 'Manage App' > 'Settings' > 'Secrets' sur Streamlit Cloud.")
    st.stop()
...
def get_rsi_class(value):
    ...
    elif value <= 30: return "oversold-cell"
    elif value >= 70: return "overbought-cell"
    return "neutral-cell"

def create_pdf_report(results_data, last_scan_time):
    """ Génère le PDF en évitant les erreurs d'encodage """
    try:
        class PDF(FPDF):
            def header(self):
        ...
        for row in results_data:
            for tf in ['H1', 'H4', 'Daily', 'Weekly']:
                ...
                if pd.notna(rsi_val):
                    ...
                    div_clean = ""
                    if "Haussi" in divergence: priority += 3; div_clean = "DIV.BULL"
                    elif "Baissi" in divergence: priority += 3; div_clean = "DIV.BEAR"
                    
                    if div_clean: signal = f"{signal} + {div_clean}" if signal else div_clean
        ...
        if opportunities:
            ...
            pdf.cell(0, 6, 'Aucun signal majeur.', 0, 1, 'L')
        pdf.ln(5)
...
if st.session_state.get('scan_done'):
    ...
    if last_scan_time:
        st.markdown(f'<div class="update-info">🔄 MAJ: {last_scan_time.strftime("%Y-%m-%d %H:%M:%S")}</div>', unsafe_allow_html=True)
...
if st.session_state.get('results'):
    ...
    st.markdown("""<div class="legend-container">
        <div class="legend-item"><div class="legend-dot oversold-dot"></div><span>Oversold (≤ 30)</span></div>
        <div class="legend-item"><div class="legend-dot overbought-dot"></div><span>Overbought (≥ 70)</span></div>
        <div class="legend-item"><span class="divergence-arrow bullish-arrow">↑</span><span>Div. Bull</span></div>
        <div class="legend-item"><span class="divergence-arrow bearish-arrow">↓</span><span>Div. Bear</span></div>
    ...
    for row in st.session_state.results:
        ...
        for tf in TIMEFRAMES_DISPLAY:
            ...
            css = get_rsi_class(rsi_val)
            val_fmt = format_rsi(rsi_val)
            icon = ""
            if div == "Haussière": icon = '<span class="divergence-arrow bullish-arrow">↑</span>'
            elif div == "Baissière": icon = '<span class="divergence-arrow bearish-arrow">↓</span>'
                
            html_table += f'<td class="{css}">{val_fmt} {icon}</td>'
        html_table += '</tr>'
